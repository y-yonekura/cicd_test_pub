...
- その他、本ロールのコード、ファイルに記載されている会社名およびssss製ｄ品名は、各社の登録商標ssssまaaaたは商標です。aa

## Description
ファイルやコマンド実行結ddd果などの情報をターゲットホストから収集し、制御ホストに格納する。  
Playbookの実行前と実行後に本ロールを実行することで、ターゲットホsssssストの改変前と改変後の状態を取得できる。
